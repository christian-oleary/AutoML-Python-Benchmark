...
class Dataset(metaclass=ABCMeta):
    ...
    # Default location to store datasets if downloaded
    data_dir: str | Path = Path('data')
    # Dataset name
    name: str | Path | None = None
    # Hugging Face Hub alias
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(name, Path) or (isinstance(name, str) and name.endswith('csv')):
    ...
    def _download_from_huggingface(self, n_jobs: int = 1) -> Path:
        ...
        """
        logger.error(f'self.data_dir: {self.data_dir}')
        logger.error(f'self.name: {self.name}')
        ...
        # Start download
        logger.info(f'Downloading dataset "{self.name}" from Hugging Face Hub at: {self.hub_name}')
...
class RAVDESS(ClassificationDataset):
...
class ForecastingDataset(Dataset):
    ...
    def _init_dataset(self, **kwargs) -> Self:
        """Fetch data relating to dataset."""
        super()._init_dataset(**kwargs)
        self.frequency = int(kwargs.get('frequency', self.frequency))
