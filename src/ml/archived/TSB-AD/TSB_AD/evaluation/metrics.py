...
def get_metrics(score, labels, slidingWindow=100, pred=None, version='opt', thre=250):
    ...
    # if pred is not None:
    #     metrics['Accuracy'] = accuracy_score(labels, pred)
    #     metrics['Balanced Accuracy'] = balanced_accuracy_score(labels, pred)
    #     metrics['MCC'] = matthews_corrcoef(labels, pred)
    #     metrics['Detection Rate'] = sum(pred) / len(pred)
    #     metrics['TPR'] = sum((pred == 1) & (labels == 1)) / sum(labels == 1) if sum(labels == 1) > 0 else 0
    #     metrics['FPR'] = sum((pred == 1) & (labels == 0)) / sum(labels == 0) if sum(labels == 0) > 0 else 0
    ###############################################################
