...
Unsupervise_AD_Pool = [
    # PHD:
    'DummyADModel', 'LunarADModel', 'PyCaretADModel', 'TimeSeriesODModel',

    # ORIGINAL:
    'TimesFM', 'TSPulse_ZS', 'Chronos', 'MOMENT_ZS', 'KShapeAD',
    'FFT', 'SR', 'Sub_IForest', 'IForest', 'LOF', 'Sub_LOF', 'POLY', 'MatrixProfile',
    'Sub_PCA', 'PCA', 'HBOS', 'Sub_HBOS', 'KNN', 'Sub_KNN','KMeansAD', 'KMeansAD_U',
    'COPOD', 'COF', 'EIF', 'RobustPCA', 'MMPAD', 'Time_RCD',

    # # 'CBLOF',  # Unstable
    # # 'Lag_Llama',  # Excluded due to incompatibility with latest PyTorch versions.
    # # 'NORMA', 'Series2Graph',  # Not included in the repository due to patents.
]
Semisupervise_AD_Pool = [
    'Left_STAMPi', 'SAND', 'MCD', 'Sub_MCD', 'OCSVM', 'Sub_OCSVM', 'AutoEncoder', 'CNN',
    'LSTMAD', 'TranAD', 'USAD', 'OmniAnomaly', 'PatchTST', 'AnomalyTransformer', 'TimesNet',
    'FITS', 'Donut', 'OFA', 'MOMENT_FT', 'M2N2', 'TSPulse_FT', 'xLSTMAD', 'CHARM',
]
# fmt: on

def run_Unsupervise_AD(model_name, data, **kwargs):
    # CHANGED: commented out the try-except for debugging
    # try:
...
################################################################################
# ADDED:
def run_DummyADModel(data, **kwargs):
    """Placeholder AD model."""
    return np.array([0.0] * len(data))

def _rescale_scores(scores):
    """Rescale inputs for AD models to [0, 1] range using MinMaxScaler."""
    return MinMaxScaler(feature_range=(0, 1)).fit_transform(scores.reshape(-1, 1)).ravel()

def run_PyCaretADModel(data, algorithm='iforest', contamination=0.1):
    """Run PyCaret anomaly detection model and return rescaled decision scores."""
    from ml.ad.anomaly_detection import PyCaretADModel
    model = PyCaretADModel(algorithm=algorithm, contamination=contamination)
    model.fit(data)
    model.predict(data)  # Ensure that decision_scores_ is populated
    scores = model.decision_function(data)
    try:
        return _rescale_scores(scores)
    except AttributeError:
        return _rescale_scores(scores.values)  # type: ignore

def run_LunarADModel(data, contamination=0.1, epochs=10, n_neighbours=5, window_size=50):
    """Run LunarAD model and return rescaled decision scores."""
    from ml.ad.anomaly_detection import LunarADModel
    model = LunarADModel(
        contamination=contamination, window_size=window_size, epochs=epochs, n_neighbours=n_neighbours
    )
    model.fit(data)
    if hasattr(model, 'decision_scores_'):
        scores = model.decision_scores_.ravel()
    else:
        scores = model.decision_function(data).ravel()
    return _rescale_scores(scores)

def run_TimeSeriesODModel(data, algorithm='IForest', window_size=50, contamination=0.1):
    """Run TimeSeriesODModel and return rescaled decision scores."""
    from ml.ad.anomaly_detection import TimeSeriesODModel
    model = TimeSeriesODModel(
        algorithm=algorithm, contamination=contamination, window_size=window_size,
    )
    model.fit(data)
    if hasattr(model, 'decision_scores_'):
        scores = model.decision_scores_.ravel()
