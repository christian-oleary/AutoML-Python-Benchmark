...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--file_list', type=str, default='../Datasets/File_List/TSB-AD-U-Tuning.csv'
    )
    ...
    for i, filename in enumerate(file_list):
        ...
        # Specify number of features, sliding window, and training data
        ...
        for index, params in enumerate(combinations):
            ...
            scores = [str(filename), str(params)] + scores
            all_results_list.append(scores)

            # Save ongoing results
            cols = ['file', 'HP'] + list(evaluation_result.keys())
            df_results = pd.DataFrame(all_results_list, columns=cols)
