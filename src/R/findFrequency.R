# Credit: https://github.com/robjhyndman/forecast
...
for(filename in data) {
  if (!grepl('0_', filename, fixed=TRUE)) {
    path = gsub(" ", "", paste(data_dir, filename))
    df = read.csv(path, sep=',', header=TRUE)
    frequency = findfrequency(df)
    # print(frequency)
    print(paste(path, frequency))
    }
}
