...
class Analysis:
    ...
    def analyze_repo(
        ...
        # self.generate_uml_diagram(repo, package_name)
        ...
        for tool, command in self.build_commands(repo.library.git_name).items():
            outputs = self._run_cli_tool(tool, command, repo, skip_existing_sca)
    ...
    def generate_uml_diagram(self, repo: GitRepo, package_name: str) -> None:
        ...
        """
        output_dir = Path(self.output_dir, 'diagrams', 'uml')
        output_dir.mkdir(parents=True, exist_ok=True)
        # target_dir = str((Path(repo.path) / package_name).resolve())
        # fmt: off
        # # UML diagrams via pyreverse
        # command = [
        #     'pyreverse', '-o', 'svg', '-p', repo.library.git_name, '-f', 'PUB_ONLY', '-k',
        #     '-d', str(output_dir), target_dir,  # '--all-ancestors', '--all-associated',
        # ]
        # logger.info(f'Generating UML diagram for {repo.library.git_name} with command: {command}')
        # self._run_subprocess_command(command, 'pyreverse', repo, verbose=True)
        # # logger.debug(f'UML saved to: {output_dir / f"classes_{repo.library.git_name}.svg"}')
        # exit()

        # Flow diagrams via code2flow recursively by subdirectory.
        output_dir = Path(self.output_dir, 'diagrams', 'flow', repo.library.git_name)
        output_dir.mkdir(parents=True, exist_ok=True)
        output_dir = str(output_dir).replace(os.sep, '/')  # type: ignore

        # Find subdirectories with Python code, excluding some directories
        subdirs = []
        for d in Path(repo.path, package_name).rglob('*'):
            # Check if the directory contains any Python files
            has_code = any(f.suffix == '.py' for f in d.rglob('*')) if d.is_dir() else False
            # Ignore directories with these names anywhere in the path
            ignored = any(
                ignored in str(d).strip('_') for ignored in [
                    'benchmarking', 'contribs', 'docker', 'docs', 'examples', 'internal',
                    'scripts', 'shell', 'test', 'tests', 'unittests'
                ]
            )
            if d.is_dir() and not ignored and has_code and not d.name.startswith('.'):
                subdirs.append(d)
        ...
        for subdir in subdirs:
            ...
            output_file = str(subdir).replace(str(repo.path), '')
            # Remove leading os.sep if present
            if output_file.startswith(os.sep):
                output_file = output_file[len(os.sep) :]
            # Replace os.sep with '__' and remove '.py' extension
            output_file = output_file.replace(os.sep, '__') + '.svg'
            logger.debug(f'{subdir} -> {output_file}')

            # Use Linux file separator for code2flow command
            subdir = str(subdir).replace(os.sep, '/')  # type: ignore

            # Explicitly list Python files as subprocess does not expand wildcards
            py_files = [str(f.resolve()).replace(os.sep, '/') for f in Path(subdir).glob('*.py')]
            if not py_files:
                logger.warning(f'Skipping {subdir}')
                continue

            # Run command to generate flow diagram for the subdirectory
            output_path = (Path(output_dir) / output_file).resolve()
            command = [
                'code2flow', '-q', *py_files, '--exclude-functions', '__init__',
                '-o', f'{output_path}',
            ]
            logger.info(f'Saving flow diagram to: {output_dir}/{output_file}')
            self._run_subprocess_command(command, 'code2flow', repo, verbose=True)

            # Remove .gv files generated by code2flow
            for gv_file in Path(output_dir).glob('*.gv'):
                if gv_file.is_file():
                    gv_file.unlink()
        # fmt: on
    ...
    def _run_cli_tool(
        ...
        """Run a CLI tool on the specified directory.
        ...
        # Run the CLI command
        logger.info(f'Running {tool} on {repo.library.git_name} using command:')
        self._run_subprocess_command(command, tool, repo, verbose)
    ...
    def _run_subprocess_command(
        self, command: list, tool: str, repo: GitRepo, verbose: bool = True
    ) -> None:
