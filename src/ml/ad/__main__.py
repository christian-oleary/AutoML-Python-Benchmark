...
def train_models(
    name: str,
    skab_data: dict[str, pd.DataFrame],
    base_results_dir_: Path,
    all_metadata: dict,
    ...
    :param dict skab_data: A dictionary mapping dataset names to DataFrames.
    :param Path base_results_dir_: The base directory to save results in.
    ...
    metadata = save_metadata(results_subdir_, dataset_name, df_, data_, window_size=window_size)
    ...
    for tool_ in ['pycaret']:
        for _, scores_ in iterate_ad_options(
...
def iterate_ad_options(
    class_name: str, results_subdir: Path, dataset_name: str, window_size: int | None, **kwargs
):
    ...
    if completion_file.exists():
        ...
        if results_file.exists():
            ...
            raise ValueError(f'Completion file exists but results file does not: {results_file}')
    ...
    for param_name, options in ad_class.parameter_options.items():
        for param_value in options:
            ...
            if combination_found(results_file, class_name, ad_class, **kwargs):
                continue

            # Fit models and calculate scores
            results = invoke_ad_class(ad_class, **kwargs)
            scores = calculate_scores(dataset_name, window_size, *results, **kwargs)
...
def combination_found(
    results_file: Path, class_name: str, ad_class: type[BaseADModel], **kwargs
) -> bool:
...
def analyse_results(
    all_results: list[dict], all_metadata: dict, results_dir: Path, plots_dir: str = 'plots'
) -> None:
    ...
    df_metadata.to_csv(results_dir / 'all_metadata.csv', index=False)
    df_metadata.T.to_csv(results_dir / 'all_metadata_T.csv', index=False)
    ...
    df.describe(include='all').transpose().to_csv(results_dir / 'all_results_summary.csv')

    # Check if there are more than one window size or library
    if df['window_size'].nunique() > 1:
        raise NotImplementedError()
    if df['model'].nunique() > 1:
        raise NotImplementedError()

    # Save latex table of results
    save_tables(df, results_dir)

    # Save box plots of test scores by the key column (e.g., dataset or model)
    save_plots(df, results_dir, plots_dir)


def save_tables(df: pd.DataFrame, results_dir: Path, min_f1_score: float = 0.5) -> None:
    ...
    df_mean = df[cols_tex].rename(columns=lambda c: readable(c))
    df_mean = df_mean.sort_values(by=['DATASET', 'MODEL NAME'])
    save_df_csv_and_tex(df_mean, results_dir / 'table_mean_unfiltered.csv')
    ...
    df_filtered = df_filtered[cols_tex].rename(columns=lambda c: readable(c))
    # Sort by dataset and model name
    df_filtered = df_filtered.sort_values(by=['DATASET', 'MODEL NAME'])
    # Save LaTeX table to file
    save_df_csv_and_tex(df_filtered, results_dir / 'table_all_scores_filtered.csv')


def readable(col_name: str) -> str:
...
def save_df_csv_and_tex(df: pd.DataFrame, csv_path: Path) -> None:
    ...
    if 'dataset' in df.columns:
        df['dataset'] = df['dataset'].apply(readable)
    if 'model_name' in df.columns:
        df['model_name'] = df['model_name'].apply(readable)

    # Readable column names for LaTeX table
    df.columns = [readable(c) for c in df.columns]
...
def save_plots(df, results_dir: Path, plots_dir: str = 'plots') -> None:
    """Save box plots of test scores by the key column (e.g., dataset or model).
    ...
    for key_col in key_cols:
        ...
        for score_col in test_score_cols:
            generate_plot(subdir, 'box', df, score_col, key_col)
...
def generate_plot(
    ...
    kwargs = {
        ...
        'xlabel': readable(x_col),
        'ylabel': readable(y_col),
        'xlabel_rotation': 90,
        'ylim': (0.0, 1.0),
    }
    for ext in ['svg']:  # , 'png']:
...
def main():
    ...
    contamination_options = [0.1, 0.2, 0.3, 0.4]
    for contamination in contamination_options:
        logger.info(f'Running experiments with contamination={contamination:.2f}...')

        base_results_dir_ = Path(f'results/ad/contamination_{contamination}')
        base_results_dir_.mkdir(parents=True, exist_ok=True)
        all_metadata = {}
        all_results = []
        # window_sizes = [2, 20]
        window_sizes = [None]  # Feature engineering
        ...
        for window_size in window_sizes:
            ...
            for name, df in skab_data.items():
                ...
                train_models(
                    name=name,
                    skab_data=skab_data,
                    base_results_dir_=base_results_dir_,
        ...
        analyse_results(all_results, all_metadata, base_results_dir_)
        logger.success(
            f'Completed experiments for contamination={contamination:.2f}. '
            f'Results saved to: {base_results_dir_}'
        )
    logger.success('Done.')
